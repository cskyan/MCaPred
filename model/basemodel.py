...
from model.BinaryClassificationLoss import BinaryClassificationLoss
...
class Linear(nn.Module):
    ...
    def __init__(self, feature_columns, feature_index, init_std=0.0001, device='cpu'):
        ...
        # varlen_sparse_feature_columns: Process feature length
    ...
    def forward(self, X, sparse_feat_refine_weight=None):
        ...
        # Embedding layer dict:construct the Embedding layer by create_embedding_dict method
        sequence_embed_dict = varlen_embedding_lookup(X, self.embedding_dict, self.feature_index,
                                                      self.varlen_sparse_feature_columns)

        # group_sparse_embedding_dict = embedding_lookup(embedding_matrix_dict, features, sparse_feature_columns) API
        varlen_embedding_list = get_varlen_pooling_list(sequence_embed_dict, X, self.feature_index,
                                                        self.varlen_sparse_feature_columns, self.device)
        # Take out an embedding sequence
...
class BaseModel(nn.Module):
    def __init__(self, linear_feature_columns, dnn_feature_columns, num_tasks, l2_reg_linear=0.01, l2_reg_embedding=0.01,
        ...
        self.awl = BinaryClassificationLoss()
